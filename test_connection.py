...
def probar_conexion():
    try:
        ...
        print("🔎 Buscando usuarios con nombre, usuario y mail...")
        ...
        usuarios_validos = [
            ...
            if hasattr(entry, "displayName") and hasattr(entry, "sAMAccountName") and hasattr(entry, "mail")
               and entry.displayName.value and entry.sAMAccountName.value and entry.mail.value
        ]

        print(f"👤 Usuarios encontrados: {len(usuarios_validos)}")
        for usuario in usuarios_validos:
            print(f"- {usuario.sAMAccountName.value} | {usuario.displayName.value} | {usuario.mail.value}")
